...
- **🔥 Firebase 실시간 저장**: Google Firebase 기반 실시간 데이터 저장
- **🔄 테이블 정렬**: 고객 목록을 이름, 등급, 생년월일 등으로 정렬 가능

## 🔥 Firebase 실시간 저장 설정 가이드
...
- 스크롤 가능한 모달창
- 간소화된 폼 레이아웃
- **🔄 모바일 자동 새로고침**: 로그인 후 서버 데이터 자동 로드
- **📱 앱 전환 감지**: 다른 앱으로 전환 후 복귀 시 자동 데이터 새로고침
- **👆 터치 상호작용**: 사용자 터치 시 서버 데이터 자동 동기화
...
### 모바일 자동 새로고침 문제
- **모바일에서 데이터 안 보임**: 로그인 후 30초 정도 대기하면 자동으로 서버 데이터 로드
- **수동 새로고침 필요**: 다른 앱으로 전환 후 다시 돌아오면 자동 새로고침 됨
- **터치로 새로고침**: 화면을 터치한 후 2초 후 자동으로 최신 데이터 로드
- **자동 새로고침 안됨**: 데스크톱 환경에서는 자동 새로고침이 비활성화됨 (모바일 전용 기능)
